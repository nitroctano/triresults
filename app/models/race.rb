class Race
  ...
  def create_entrant racer
    new_ent=Entrant.new
    new_ent.race=self.attributes.symbolize_keys.slice(:_id,:n,:date)
    new_ent.racer=racer.info.attributes
    new_ent.group=get_group(new_ent.racer)
    events.each { |event|
      new_ent.send("#{event.name}=", event)
    }
    if new_ent.validate
      new_ent.bib=next_bib
      new_ent.save
      self.save
    end
    new_ent
  ...
  def self.upcoming_available_to(racer)
    ...
    self.upcoming.not.in(:id=>upcoming_racer_races_ids)
  end
